...
@app.route("/comprar_agora/<int:evento_id>")
def comprar_agora(evento_id):
    ...
    # Pega todos os lugares disponíveis do evento (não vendidos)
    lugares = Lugar.select().where((Lugar.evento == evento) & (Lugar.vendido == False))

    if not lugares:
        flash("Não há lugares disponíveis para este evento.", "warning")
        return redirect(url_for("eventos"))

    return render_template(
        "checkout_comprar_agora.html",
        evento=evento,
        lugares=lugares,
    )

@app.route("/finalizar_compra_agora/<int:evento_id>", methods=["POST"])
def finalizar_compra_agora(evento_id):
    if g.utilizador is None:
        flash("Precisa de iniciar sessão.", "warning")
        return redirect(url_for("login"))
    ...
    if evento.tipo == "Concerto":
        quantidade = int(request.form.get("quantidade", 0))
        lugares_disponiveis = Lugar.select().where((Lugar.evento == evento) & (Lugar.tipo == "pista") & (Lugar.vendido == False))
        if quantidade < 1 or quantidade > lugares_disponiveis.count():
            flash("Quantidade inválida.", "warning")
            return redirect(url_for("comprar_agora", evento_id=evento.id))

        total = 0
        lugares = []
        for lugar in lugares_disponiveis.limit(quantidade):
            lugar.vendido = True
            lugar.save()
            total += float(lugar.preco_base)
        ...
        for lid in selecionados:
            lugar = Lugar.get_or_none(Lugar.id == int(lid), Lugar.vendido == False)
            if not lugar:
                flash("Um dos lugares já não está disponível.", "danger")
                return redirect(url_for("comprar_agora", evento_id=evento.id))
            lugar.vendido = True
            lugar.save()
            total += float(lugar.preco_base)
    ...
    venda = Venda.create(utilizador=g.utilizador, evento=evento, total=total)

    for lugar in lugares:
        Bilhete.create(venda=venda, lugar=lugar, preco=lugar.preco_base)

    Recibo.create(venda=venda, nif=g.utilizador.documento_identificacao, valor_total=total)

    flash(f"Compra realizada com sucesso! Total: {total} €", "success")
